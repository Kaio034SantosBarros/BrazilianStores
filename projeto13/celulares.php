  <?php
    ...
    <div style="border: 3px solid black; margin-top: 1%; background-color: #BEBEBE;">  
      <h3 class="text-center"><marquee scrollDelay=1><font color=black><DATA>Celulares disponíveis</DATA></em></font></marquee></h3>
    </div>
    ...
    <div style="margin-left: 3%;">
      ...
      <?php 
        ...
        $query2 = "select * from produto  where categoria = 'celulares'";
        $consulta2 = mysqli_query($con, $query2);
        $total = mysqli_num_rows($consulta2);

        $query = "select * from produto where categoria = 'celulares' limit $inicial, $exibir";
  ...
  <?php echo "<div class='text-center'>"; 
    echo "<div><a class='btn btn-primary' href='celulares.php?pi=0&exibir=$exibir'>Primeiro</a>";
        for($i = 1; $i <= $paginas; $i++){
         if($i == 1){
          $pi = 0;
           }else{
        ...
        echo "<a class='btn btn-primary' href='celulares.php?pi=$pi&exibir=$exibir'>$i</a>";
        }
        echo "<a class='btn btn-primary' href='celulares.php?pi=$pi&exibir=$exibir'>ultimo</a></div>";
        echo "</div>"
  ?>  
  <footer>
